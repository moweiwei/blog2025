---
updateTime: "2024-07-12 20:45"
date: "2024-07-12"
title: "前端工程师面试题2"
desc: "HTTP、浏览器与性能相关的进阶面试题整理。"
...
- 文档分为“协议 / 浏览器 / 性能 / 安全 / 框架”五大板块，推荐在浏览器搜索 `##` 或使用 VitePress 目录定位到对应章节。
- 每道题都附了“参考回答”，先自己写再对照更能检验掌握程度；必要时把答案整理成卡片，方便复习。
- 题目较长，可结合 `⌘ + F` 搜关键词，例如 “缓存” 或 “事件循环”，快速找到对应的小节。
...
提示：可以将提示文字夹在 iframe 标签之间，来提示某些不支持 iframe 的浏览器
...
1. 粗暴型，禁用缩放：在 `<meta name="viewport" content="width=device-width, user-scalable=no">` 中显式设置 `user-scalable=no`。
2. 利用 FastClick，其原理是：
...
样式表的来源不同时，优先级顺序为：内联样式内部样式 \外部样式 \浏览器用户自定义样式 \浏览器默认样式
...
*（原文配图：image13.jpeg）* *（原文配图：image14.jpeg）*
...
*（原文配图：image17.jpeg）* 参考回答：

- ### 固定定位布局键盘挡住输入框内容

参考回答：

*（原文配图：image18.jpeg）* *（原文配图：image19.jpeg）*

- ### click 的 300ms 延迟问题和点击穿透问题

*（原文配图：image20.jpeg）* 参 考 回 答 ：
...
*（原文配图：image42.jpeg）* Vue.js 2.x 的数据更新并触发重新渲染的粒度是组件级的，单个组件内部 需要遍历该组件的整个 vnode 树。在 2.0
...
中间件提供第三方插件的模式，自定义拦截 action -reducer 的过程。变为action -middlewares -reducer 。这种机制可以让我们改变数据流，实现如异步action
